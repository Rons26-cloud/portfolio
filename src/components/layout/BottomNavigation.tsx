...
export function BottomNavigation() { return <nav className="bottom-nav" aria-label="Quick navigation">{items.map(({ label, href, Icon }) => <Link key={href} href={href}><Icon size={19} /><span>{label}</span></Link>)}</nav>; }
